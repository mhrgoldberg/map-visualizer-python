...
def all_routes() -> dict:
    ...
    """
    return {
        "dict": {track.id: track.to_simple_dict() for track in current_user.tracks},
...
def get_track(track_id: int) -> dict:
    ...
    """
    track = Track.query.options(joinedload(Track.track_points)).get(track_id)
    if track is None or track.user_id != current_user.id:
        return {"errors": {"track": "Sorry, this track does not exist."}}, 404
    return {"dict": {track.id: track.to_dict()}, "ordering": [track.id]}
...
def create_track() -> dict:
    ...
    if form.validate_on_submit():
        try:
            ...
            return {
                "dict": {track.id: track.to_simple_dict() for track in new_tracks},
                "ordering": [track.id for track in current_user.tracks],
            }, 201
